...
describe("SquaddieActionValidationService", () => {
    ...
    describe("getAllValidTargetsInRangeOfAction", () => {
        it("returns actor at current coordinate for self-only action", () => {
            ...
            expect(result.get(actorCoordinateKey)!.has(actorSquaddieKey)).toBe(
                true
            )
        })
        ...
        it("returns actor and nearby ally for self-and-friend melee, excludes distant ally", () => {
            ...
            const nearAllyCoordinateKey =
                OffsetCoordinateService.coordinateToKey({ row: 0, col: 1 })
            const farAllyCoordinateKey =
                OffsetCoordinateService.coordinateToKey({ row: 0, col: 3 })
        ...
        it("returns foe only for foe-targeting ranged action, excludes ally and self", () => {
            ...
            expect(result.get(enemyCoordinateKey)!.has(enemySquaddieKey)).toBe(
                true
            )
        })
